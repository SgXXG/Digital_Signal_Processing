...
export default function AmplitudeModulation () {
  ...
  }, [N, A1, A2, f1, f2, f01, f02,k]);

  useEffect(() => {
    const filteredData = getfilteredTimeData()
    setFilteredTimeData(filteredData)
  }, [N, A1, A2, f1, f2, f01, f02, k, BigK, selectedTimeFilter])
  ...
  }, [N, A1, A2, f1, f2, f01, f02,k, slice, lowSlice, highSlice, selectedFreqFilter])

  const harmonicSum = useMemo(() => {
    const dataArr = []
    for (let i = 0; i < N; i++) {
      const value = A1 * Math.sin(2 * i * Math.PI * f1/N + f01) *
      2 * A2 / Math.PI * Math.asin(Math.sin(2 * Math.PI * f2 * i / N + f02))
      dataArr.push(value) 
    }
    return dataArr
  ...
  const calckpoints = useMemo(() => {
    ...
    for (let i = 0; i < N; i += N / k) {
      const value = A2 * Math.sin(2 * i * Math.PI * f2 / N + f02) *
      2 * A1 / Math.PI * Math.asin(Math.sin(2 * Math.PI * f1 * i / N + f01))
      dataArr.push(value)
    }
    console.log(dataArr)
    return dataArr
          ...
          <div className="params-container">
            <div className="params-box">
              ...
              <div className="params-signal">
                <p className='param-title'>Несущий сигнал</p>
                <div className="params-box">
              ...
              <div className="params-signal">
                <p className='param-title'>Модулирующий сигнал</p>
                <div className="params-box">
                  <div className="param-name">Amplitude:</div>
