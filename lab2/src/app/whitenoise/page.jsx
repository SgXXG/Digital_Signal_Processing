...
export default function Triangle() {
  ...
  const fourier = useMemo(() => calcFourier(kpoints, x.length, trigResults), [A, k]);

  const reversePoints = useMemo(() => {
    const reverse = calcReverseFourier(fourier, x.length);
    return reverse;
  }, [A, k]);
  const xDataset1 = [];
  const xDataset2 = [];
